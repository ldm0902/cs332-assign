...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case x #:: tail => {
      val newExplored = explored+x._1
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(x._1, x._2), newExplored)
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] =  from(Stream((startBlock,Nil)),Nil.toSet)
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = {
    for (path <- pathsFromStart
         if done(path._1)) yield path
  }
