...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p =Promise[T]
      for(f<-fs) f.onComplete(p.tryComplete)
      p.future
    }

    /** Returns a future with a unit value that is completed after time `t`.
     */
    def delay(t: Duration): Future[Unit] = Future[Unit](Thread.sleep(t.toMillis))
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val cnclTknSrc = CancellationTokenSource()
      f(cnclTknSrc.cancellationToken)
      cnclTknSrc
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = f.value match{
      //Try(Await.result(future, Duration.Zero)).getOrElse(throw new NoSuchElementException())
      //Does 'Duration.Zero' mean non-blocking?
